...
- [ ] T004 [US1] Implement makeMove function that places current player's mark in an empty cell, rejects occupied cells (FR-004), and switches currentPlayer in game.js
...
- [ ] T010 [US2] Implement updateStatus function that sets #status textContent to turn indicator ("X's turn"/"O's turn"), win announcement ("X wins!"/"O wins!"), or draw ("It's a draw!") in game.js
...
- [ ] T014 [P] Implement arrow-key navigation between grid cells within #board (move focus on ArrowUp/Down/Left/Right, wrap optional) in game.js
- [ ] T015 [P] Verify and refine ARIA attributes: ensure cell aria-labels update on state change, #status aria-live announces changes, and all cells disable on game over in game.js
...
3. T014+T015 (parallel) → T016 → **done**
